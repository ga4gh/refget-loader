...
def upload(**kwargs):
    ...
    def parse_manifest(manifest_path):
        ...
        config = manifest_file.readline().split(":")[1].strip()
        ...
        return [config, seq_table, additional_table]

    # parse the manifest file using the above method
    manifest = kwargs["manifest"]
    config, seq_table, additional_table = \
    ...
    destination_obj = config_obj["destination"]
    destination_type = destination_obj["type"]
    upload_method = DESTINATIONS[destination_type]
